...
// API endpoint for sending order email & syncing to Google Sheet
app.post('/send-order', async (req, res) => {
  ...
  if (items && items.length > 0) {
    itemsHtml = items.map(item => 
      <tr style=border-bottom: 1px solid #eee;>
        <td style=padding: 10px 8px;><strong>x</strong> </td>
        <td style=padding: 10px 8px; text-align: right; color: #D85A2A; font-weight: bold;>đ</td>
      </tr>
    ).join('');
  } else {
  ...
  const emailHtml = 
    <div style=font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden;>
      ...
      <div style=padding: 24px;>
        <p style=font-size: 16px; color: #333;>Xin chào <strong></strong>,</p>
        ...
        <div style=background: #FDF8F5; border: 1px dashed #D85A2A; border-radius: 8px; padding: 16px; margin: 20px 0;>
          <h3 style=margin: 0 0 12px 0; color: #D85A2A; font-size: 17px;>MÃ ĐƠN HÀNG: #LN-</h3>
          <p style=margin: 4px 0; color: #444;><strong>Người nhận:</strong> </p>
          <p style=margin: 4px 0; color: #444;><strong>Số điện thoại:</strong> </p>
          <p style=margin: 4px 0; color: #444;><strong>Địa chỉ giao:</strong> </p>
          <p style=margin: 4px 0; color: #444;><strong>Dịch vụ mượn bếp cồn:</strong> </p>
        </div>
        ...
        <div style=text-align: right; margin-top: 20px; padding-top: 12px; border-top: 2px solid #eee;>
          ...
          <span style=font-size: 22px; font-weight: bold; color: #D85A2A;></span>
        </div>
  ...
  try {
    await transporter.sendMail({
      ...
      to: cust_email,
      subject: [Lẩu Nhà] Xác nhận đơn hàng # - ,
      html: emailHtml
    });
    ...
    if (googleSheetUrl) {
      ...
      }).catch(err => console.error('Sheet err:', err.message));
    }
