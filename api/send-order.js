...
async function sendSmtpEmail(to, subject, html) { const nodemailer = require('nodemailer'); const user = process.env.SMTP_USER || DEFAULT_SMTP_USER; const pass = process.env.SMTP_PASS || DEFAULT_SMTP_PASS; const transporter = nodemailer.createTransport({ host: 'smtp.gmail.com', port: 465, secure: true, auth: { user, pass } }); await transporter.sendMail({ from: process.env.FROM_EMAIL || user, to, subject, html }); }
function sendEmail(to, subject, html) { if (process.env.SMTP_PASS || DEFAULT_SMTP_PASS) return sendSmtpEmail(to, subject, html); if (process.env.RESEND_API_KEY) return sendResendEmail(to, subject, html); return Promise.reject(new Error('Neither SMTP_PASS nor RESEND_API_KEY is configured')); }
