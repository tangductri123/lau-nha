...
function itemRows(order) { return order.items.map(item => `<tr><td style="padding:10px 0;border-bottom:1px solid #eee">${escapeHtml(item.name)}</td><td style="padding:10px 0;border-bottom:1px solid #eee;text-align:center">${item.qty}</td><td style="padding:10px 0;border-bottom:1px solid #eee;text-align:right">${formatVnd(item.price * item.qty)}</td></tr>`).join(''); }
...
function buildCustomerEmail(order) { const stove = order.stoveFee ? `Bếp cồn: ${formatVnd(order.stoveFee)}` : 'Bếp cồn: Miễn phí'; return emailShell(`Xác nhận đơn hàng #${escapeHtml(order.orderCode)}`, `<p>Xin chào <strong>${escapeHtml(order.name)}</strong>,</p><p>Lẩu Mang Đi đã nhận được đơn hàng của bạn.</p><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding-bottom:8px">Món</th><th style="padding-bottom:8px">SL</th><th style="text-align:right;padding-bottom:8px">Thành tiền</th></tr></thead><tbody>${itemRows(order)}</tbody></table><p style="line-height:1.8"><strong>Tạm tính:</strong> ${formatVnd(order.subtotal)}<br><strong>Giảm giá:</strong> -${formatVnd(order.discount)}<br><strong>${stove}</strong><br><span style="font-size:20px;color:#a52a16"><strong>Tổng cộng: ${formatVnd(order.total)}</strong></span></p><div style="background:#fff8f0;padding:16px;border-radius:10px;line-height:1.8"><strong>Giao đến:</strong> ${escapeHtml(order.address)}<br><strong>Số điện thoại:</strong> ${escapeHtml(order.phone)}${order.notes ? `<br><strong>Ghi chú:</strong> ${escapeHtml(order.notes)}` : ''}</div><p>Nhân viên sẽ liên hệ để xác nhận. Nếu có bếp, vui lòng chuẩn bị mặt bằng; thời gian setup dự kiến khoảng 15 phút.</p>`); }
function buildStoreEmail(order) { return emailShell(`🔥 Đơn hàng mới #${escapeHtml(order.orderCode)} - ${escapeHtml(order.name)} (${formatVnd(order.total)})`, `<h2 style="margin-top:0;color:#a52a16">🔥 Đơn hàng mới</h2><div style="line-height:1.9"><strong>Khách hàng:</strong> ${escapeHtml(order.name)}<br><strong>Điện thoại:</strong> ${escapeHtml(order.phone)}<br><strong>Địa chỉ:</strong> ${escapeHtml(order.address)}<br><strong>Email:</strong> ${escapeHtml(order.email)}<br><strong>Bếp cồn:</strong> ${order.stoveFee ? `Có - ${formatVnd(order.stoveFee)}` : 'Không/miễn phí'}${order.notes ? `<br><strong>Ghi chú:</strong> ${escapeHtml(order.notes)}` : ''}<br><strong>Thời gian:</strong> ${escapeHtml(order.createdAt)}</div><hr style="border:0;border-top:1px solid #eee;margin:20px 0"><table style="width:100%;border-collapse:collapse"><tbody>${itemRows(order)}</tbody></table><p style="text-align:right;font-size:20px;color:#a52a16"><strong>Tổng: ${formatVnd(order.total)}</strong></p>`); }
async function sendResendEmail(to, subject, html) { const apiKey = process.env.RESEND_API_KEY; if (!apiKey) throw new Error('RESEND_API_KEY is not configured'); const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: process.env.FROM_EMAIL || DEFAULT_FROM_EMAIL, to: [to], subject, html }) }); if (!response.ok) throw new Error(`Resend returned HTTP ${response.status}: ${(await response.text().catch(() => '')).slice(0, 500)}`); }
async function sendSmtpEmail(to, subject, html) { const nodemailer = require('nodemailer'); const user = process.env.SMTP_USER || DEFAULT_SMTP_USER; const pass = process.env.SMTP_PASS; if (!pass) throw new Error('SMTP_PASS is not configured'); const transporter = nodemailer.createTransport({ host: 'smtp.gmail.com', port: 465, secure: true, auth: { user, pass } }); await transporter.sendMail({ from: process.env.FROM_EMAIL || user, to, subject, html }); }
function sendEmail(to, subject, html) { if (process.env.SMTP_PASS) return sendSmtpEmail(to, subject, html); if (process.env.RESEND_API_KEY) return sendResendEmail(to, subject, html); return Promise.reject(new Error('Neither SMTP_PASS nor RESEND_API_KEY is configured')); }
function buildTelegramMessage(order) { const lines = order.items.map(item => `• ${item.qty}x ${escapeHtml(item.name)} — ${formatVnd(item.price * item.qty)}`); return ['🍲 ĐƠN HÀNG MỚI — LẨU NHÀ', `Mã đơn: #LN-${escapeHtml(order.orderCode)}`, `Khách hàng: ${escapeHtml(order.name)}`, `Điện thoại: ${escapeHtml(order.phone)}`, `Email: ${escapeHtml(order.email)}`, `Địa chỉ: ${escapeHtml(order.address)}`, '', 'Chi tiết:', ...lines, `Tạm tính: ${formatVnd(order.subtotal)}`, `Bếp cồn: ${order.stoveFee ? formatVnd(order.stoveFee) : 'Miễn phí/Không mượn'}`, `Giảm giá: -${formatVnd(order.discount)}`, `TỔNG: ${formatVnd(order.total)}`].join('\\n'); }
async function sendTelegramMessage(message) { const token = process.env.TELEGRAM_BOT_TOKEN || DEFAULT_TELEGRAM_BOT_TOKEN; const chatId = process.env.TELEGRAM_CHAT_ID || DEFAULT_TELEGRAM_CHAT_ID; const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: message, disable_web_page_preview: true }) }); if (!response.ok) throw new Error(`Telegram returned HTTP ${response.status}: ${(await response.text().catch(() => '')).slice(0, 500)}`); }
module.exports = async (req, res) => {
  ...
  try {
    ...
    const customerEmailPromise = sendEmail(order.email, `[Lẩu Mang Đi] Xác nhận đơn hàng #${order.orderCode}`, buildCustomerEmail(order));
    const storeEmailPromise = sendEmail(process.env.STORE_EMAIL || DEFAULT_STORE_EMAIL, `🔥 Đơn hàng mới #${order.orderCode} - ${order.name} (${order.total.toLocaleString('vi-VN')}đ)`, buildStoreEmail(order));
